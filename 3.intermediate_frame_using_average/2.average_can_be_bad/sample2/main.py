...
def png_intermediate(png1_path, png2_path, png_inter_path):
    # 1. Load images with IMREAD_UNCHANGED to keep all channels (including Alpha/PNG transparency)
    ...
    # 2. Convert to 16-bit integers to prevent 8-bit overflow during addition
    img1_16 = img1.astype(np.uint16)
    img2_16 = img2.astype(np.uint16)

    # 3. Take the absolute integer average (exactly like integer math in C)
    # Using '//' ensures C-style floor integer division
    average_16 = (img1_16 + img2_16) // 2

    # 4. Cast back down to standard 8-bit image format with zero post-processing
    final_img = average_16.astype(np.uint8)

    # 5. Save the output PNG
