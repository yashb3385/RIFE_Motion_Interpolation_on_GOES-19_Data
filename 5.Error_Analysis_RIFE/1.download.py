...
from utils.intermediate_time import inter_time
            ...
            with requests.get(file_url, stream=True) as r:
                ...
                with open(os.path.join(download_dir, filename), "wb") as file:
                    for chunk in r.iter_content(chunk_size=8192):
                        file.write(chunk)
            print(f"   -> Successfully saved : {os.path.join(download_dir, filename)}")
...
# 1st frame
year1, day1, hr1, min1 = 2025, 1, 1, 10

# 2nd frame
year2, day2, hr2, min2 = 2025, 1, 1, 30

year_inter, day_inter, hr_inter, min_inter = inter_time(year1, day1, hr1, min1, year2, day2, hr2, min2)

for i in channels_list:
    download_goes_file(year1, day1, hr1, min1, i, f"input/C{i:02d}")
    download_goes_file(year2, day2, hr2, min2, i, f"input/C{i:02d}")
    download_goes_file(year_inter, day_inter, hr_inter, min_inter, i, f"intermediate/C{i:02d}")
    print(f"\n [>] Completed with Channel {i} data.\n")
